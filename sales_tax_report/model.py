...
class SampleDevelopmentReport(models.AbstractModel):
    ...
    def render_html(self,docids, data=None):
        ...
        records_opening = self.env['account.invoice'].search([('date_invoice', '<', form)])

        out_tax = 0
        in_tax = 0
        for x in records_opening:
            if x.type == 'out_invoice':
                for y in x.tax_line_ids:
                    if y.name == 'Sales Tax @ 17%' or y.name == 'Additional Sales Tax @ 3%':
        ...
        for x in records:
            if x.type == 'in_invoice':
                for y in x.tax_line_ids:
                    if y.name == 'Sales Tax @ 17%':
                        vendor_sales = vendor_sales + y.amount

        for x in records:
            if x.type == 'in_invoice':
                for y in x.tax_line_ids:
                    if y.name == 'Additional Sales Tax @ 3%':
